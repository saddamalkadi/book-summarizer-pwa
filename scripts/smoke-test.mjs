...
async function checkDesktop() {
  ...
  if (!/8\.85/.test(title)) fail(`Title missing v8.85: got "${title}"`);
  else log('OK  title contains v8.85:', title);

  const verLabel = await page.textContent('#sideVersionLabel').catch(() => '');
  if (!/8\.85/.test(verLabel || '')) fail(`sideVersionLabel != v8.85: got "${verLabel}"`);
