...
async function checkDesktop() {
  ...
  if (!/8\.84/.test(title)) fail(`Title missing v8.84: got "${title}"`);
  else log('OK  title contains v8.84:', title);

  const verLabel = await page.textContent('#sideVersionLabel').catch(() => '');
  if (!/8\.84/.test(verLabel || '')) fail(`sideVersionLabel != v8.84: got "${verLabel}"`);
  ...
  // Try downloads page
  await page.locator('[data-page="downloads"]').first().click({ trial: true }).catch(() => {});
