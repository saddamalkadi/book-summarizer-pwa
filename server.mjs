...
async function handleGoogleTtsProxy(request){
  try{
    ...
    const APP_ADMIN_EMAIL      = (process.env.APP_ADMIN_EMAIL || '').trim();
    const APP_UPGRADE_EMAIL    = (process.env.APP_UPGRADE_EMAIL || APP_ADMIN_EMAIL).trim();
    const GOOGLE_CLIENT_ID_WEB = (process.env.GOOGLE_CLIENT_ID_WEB || '').trim();
    const APP_BRAND_NAME       = (process.env.APP_BRAND_NAME || 'AI Workspace Studio').trim();
    const APP_DEVELOPER_NAME   = (process.env.APP_DEVELOPER_NAME || '').trim();
    ...
    const workerBindings = [
      ...
      {type:'service',name:'CONVERT',service:CONVERT_SERVICE,environment:'production'},
      {type:'plain_text',name:'APP_BRAND_NAME',text:APP_BRAND_NAME},
