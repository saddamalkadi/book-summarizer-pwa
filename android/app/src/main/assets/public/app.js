...
function refreshDeepSearchBtn(){
  ...
  function explainAuthError(error, { nativeGoogle = false } = {}){
    ...
    if (/AUTH_ADMIN_PASSWORD_NOT_CONFIGURED|Admin password login is not configured/i.test(raw)){
      return 'دخول الإدارة بكلمة المرور غير مفعل حاليًا. يرجى تسجيل الدخول باستخدام Google.';
    }
    ...
    // Do not leak raw backend errors to end users.
    if (/^\s*(Error|TypeError|SyntaxError|ReferenceError|RangeError)[:\s]/i.test(raw) ||
        /\b\d{3}\b/.test(raw) ||
        /worker|cloudflare|kv_namespace|binding|fetch|CORS|NetworkError/i.test(raw)){
  ...
  function buildGuideSections(){
    return [
      ...
      },
      {
        ...
        body: 'يوجد قسم تنزيلات داخل الإعدادات يتيح تنزيل تطبيق Android بصيغة APK مباشرة. يمكن تثبيت التطبيق كـ PWA من المتصفح أيضًا.',
        steps: [
      ...
      overview.innerHTML = `
        <div class="bubble app-dl-card" style="margin:0;padding:16px">
          ...
          <div class="actions" style="flex-wrap:wrap;gap:8px;margin-top:8px">
            <a class="btn ghost sm" href="${APK_BACKUP_URL}" target="_blank" rel="noopener noreferrer">رابط التنزيل الاحتياطي</a>
          </div>
