/* AI Workspace Studio v2 - strategic platform skeleton (no build step) */
(() => {
  'use strict';
  ...
  // ---------------- Storage ----------------
  const KEYS = {
    settings: 'aistudio_settings_v2',
    projects: 'aistudio_projects_v2',
    curProject: 'aistudio_cur_project_v2',
    threads: (pid) => `aistudio_threads_${pid}_v2`,
    curThread: (pid) => `aistudio_cur_thread_${pid}_v2`,
    files: (pid) => `aistudio_files_${pid}_v2`,
    canvas: (pid) => `aistudio_canvas_${pid}_v2`,
    downloads: 'aistudio_downloads_v2',
    modeDeep: 'aistudio_mode_deep_v2',
    modeAgent: 'aistudio_mode_agent_v2',
    webToggle: 'aistudio_webtoggle_v2',
    modelCache: 'aistudio_or_models_cache_v2',
    favorites: 'aistudio_model_favs_v2',
    recent: 'aistudio_model_recent_v2'
  };
...
أعد JSON فقط بدون شرح.` }
  ...
  function curCanvasId(pid){ return localStorage.getItem(`aistudio_canvas_cur_${pid}_v2`) || ''; }
  function setCurCanvasId(pid, id){ localStorage.setItem(`aistudio_canvas_cur_${pid}_v2`, id || ''); }
  ...
  function deleteProject(){
    ...
    try{ localStorage.removeItem(`aistudio_canvas_cur_${cur.id}_v2`); }catch(_){}
  ...
  function setActiveNav(page){
    ...
    const titles = { chat:'الدردشة', canvas:'كانفس', files:'الملفات', downloads:'التحميلات', projects:'المشاريع', settings:'الإعدادات' };
