...
$('baseUrl') && ($('baseUrl').value = saved.baseUrl);
  ...
  async function callChatCompletions({ apiKey, baseUrl, model, messages, max_tokens=1400, temperature=0.25, plugins=null }){
    const res = await fetch(`${baseUrl}/chat/completions`, {
      method:'POST',
      headers:{ 'Content-Type':'application/json', 'Authorization':`Bearer ${apiKey}` },
      body: JSON.stringify(Object.assign({ model, messages, max_tokens, temperature }, plugins ? { plugins } : {}))
    });
    if (!res.ok){
      const txt = await res.text().catch(()=> '');
