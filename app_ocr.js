...
function globalClickRouter(e){
  const t = e.target?.closest?.('#chatDeepBtn, #chatAgentBtn, #chatResearchBtn');
  if (!t) return;
  if (t.id === 'chatDeepBtn'){ setDeepMode(!isDeepMode()); updateChatModeButtons(); toast(isDeepMode() ? '🧠 تم تفعيل التفكير العميق' : '🧠 تم إيقاف التفكير العميق'); return; }
  if (t.id === 'chatAgentBtn'){ setAgentMode(!isAgentMode()); updateChatModeButtons(); toast(isAgentMode() ? '🤖 تم تفعيل وضع الوكيل' : '🤖 تم إيقاف وضع الوكيل'); return; }
  if (t.id === 'chatResearchBtn'){ try{ openResearch(true); }catch(_){ } return; }
}
