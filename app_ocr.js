/* Book Summarizer Pro v8 — OCR + DOCX + Chat Threads + Persistence (IndexedDB) */
(() => {
  const APP_VER = 8;
  ...
  function persistDraft(){
    ...
    localStorage.setItem('draft_v8', JSON.stringify(payload));
  }

  function restoreDraft(){
    try{
      const raw = localStorage.getItem('draft_v8');
      if (!raw) return;
  ...
  function persistChatDraft(){
    ...
    localStorage.setItem('chat_draft_v8', JSON.stringify(payload));
  }

  function restoreChatDraft(){
    try{
      const raw = localStorage.getItem('chat_draft_v8');
      if (!raw) return;
      const d = JSON.parse(raw);
...
<script>setTimeout(()=>window.print(), 450);</script>
  ...
  function getHtmlToDocxFn(){
    // turbodocx browser build exposes HTMLToDOCX
  ...
  async function exportDocx(filename, title, text){
    ...
    if (!fn) throw new Error('مكتبة DOCX لم تُحمّل. تأكد من إضافة html-to-docx.browser.js + polyfills في index.html.');
    ...
    // Signature: HTMLToDOCX(html, header, options, footer)
