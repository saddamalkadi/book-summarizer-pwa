...
(() => {
  ...
  function enhanceCanvas(canvas){
    // تحسين بسيط: grayscale + contrast + threshold خفيف
  ...
  function buildChatMessagesForAPI(userQuestion){
    ...
    // نرسل آخر 14 رسالة فقط لتقليل الحجم
  ...
  // Draft persistence (localStorage) for quick restore
...
<script>setTimeout(()=>window.print(), 450);</script>
  ...
  async function updateRecord(id, patch){
    ...
    if (!rec) return null;
    const next = { ...rec, ...patch, updatedAt: nowTs() };
    await dbPut(next);
    return next;
  }
  ...
  async function renderHistory(){
    ...
    items.forEach(item => {
      ...
      btnExportDocx.onclick = async () => {
        try{
          const title = item.title || typeLabel(item.type);
          if (item.type === 'chat'){
            const txt = (item.meta?.messages || []).map(m => `${m.role==='user'?'أنت':'المساعد'}: ${m.content}`).join('\n\n');
            await exportDocx(`${title}.docx`, title, txt);
          } else {
            await exportDocx(`${title}.docx`, title, item.content || '');
          }
  ...
  function init(){
    ...
    $('btnExport')?.addEventListener('click', () => openExport(true));
    $('btnCloseExport')?.addEventListener('click', () => openExport(false));
    $('overlay')?.addEventListener('click', () => { openSettings(false); openExport(false); });
