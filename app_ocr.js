...
async function callGeminiStream({ geminiKey, geminiModel, systemInstructionText, contents, temperature=0.25, maxOutputTokens=4096, signal, onDelta }){
  ...
  return { text: (combined||'').trim(), finishReason };
}
...
$('baseUrl') && ($('baseUrl').value = saved.baseUrl);
  ...
  async function callGeminiGenerateContent({ geminiKey, geminiModel, systemInstructionText, contents, temperature=0.25, maxOutputTokens=4096, autoContinue=true, maxParts=6 , signal}){
...
let currentChatAbort = null;
  ...
  function renderChat(){
    ...
    chatMessages.forEach(m => {
      ...
      if (m.role !== 'user') enhanceCodeBlocks(body);
  ...
  function getChatSettingsOrThrow(){
    ...
    if (chatProvider === 'gemini'){
      ...
      const geminiModel = ($('geminiModel')?.value || 'gemini-2.0-flash').trim();
      ...
      return { chatProvider, geminiKey, geminiModel, language, chatDirectFiles, visionMode ,
      chatMaxOutTokens,
      chatMaxParts,
      chatHistoryTurns,
      chatStreaming
    ...
    async function buildUserPartsForChat(){
      ...
      if (useAttachments){
        for (const att of chatAttachments){
          ...
          // Other files: send the file itself as Base64 (no upload needed) — fallback to extracted text if too big
          const MAX_DIRECT_FILE_BYTES = 12 * 1024 * 1024; // 12MB per file for mobile safety
          let fileData = null;
          if (att.file && att.file.size <= MAX_DIRECT_FILE_BYTES){
            try{ fileData = await fileToDataUrl(att.file); }catch(e){ fileData = null; }
          }
          if (fileData){
            parts.push({ type:'file', file:{ filename: att.name, file_data: fileData } });
...
<script>setTimeout(()=>window.print(), 450);</script>
  ...
  function downloadBlob(filename, blob){
    ...
    // في بعض الأجهزة، التأخير البسيط يجعل التنزيل أكثر موثوقية
    setTimeout(() => {
      try{ a.click(); }catch(_){}
    }, 50);

    // لا تحذف الرابط مباشرة حتى لا يفشل التنزيل
    setTimeout(() => {
