...
// Robust click routing (mobile safe)
function globalClickRouter(e){
  // Mobile-safe: text nodes may be the event target
  const node = (e && e.target && e.target.nodeType === 3) ? e.target.parentElement : e.target;
  const t = node?.closest?.('#chatDeepBtn, #chatAgentBtn, #chatResearchBtn, #chatModesResetBtn');
  if (!t) return;
  ...
  if (t.id === 'chatDeepBtn'){
    ...
    try{ updateChatModeButtons(); }catch(_){}
    try{ updateChatModelBtn(); }catch(_){}
    try{ updateUsageBar(); }catch(_){}
    try{ renderChatModelMenu(); }catch(_){}
    toast(isDeepMode() ? '🧠 تم تفعيل التفكير العميق' : '🧠 تم إيقاف التفكير العميق');
    return;
  }

  if (t.id === 'chatAgentBtn'){
    setAgentMode(!isAgentMode());
    try{ updateChatModeButtons(); }catch(_){}
    try{ updateChatModelBtn(); }catch(_){}
    try{ updateUsageBar(); }catch(_){}
    try{ renderChatModelMenu(); }catch(_){}
    toast(isAgentMode() ? '🤖 تم تفعيل وضع الوكيل' : '🤖 تم إيقاف وضع الوكيل');
    return;
  }

  if (t.id === 'chatResearchBtn'){
    try{ openResearch(true); }catch(_){}
    return;
  }
}
...
function disableAllModes(){
  setDeepMode(false);
  setAgentMode(false);
  // If current model is :online, revert to base model
  try{
    const mi = $('model');
    if (mi && String(mi.value||'').includes(':online')){
      mi.value = String(mi.value).replace(':online','');
      saveSettings();
    }
  }catch(_){}
  applyModeUIRefresh();
...
function toggleDeepModeNow(){
  setDeepMode(!isDeepMode());
  applyModeUIRefresh();
  toast(isDeepMode() ? '🧠 تم تفعيل التفكير العميق' : '🧠 تم إيقاف التفكير العميق');
}

function toggleAgentModeNow(){
  setAgentMode(!isAgentMode());
  applyModeUIRefresh();
  toast(isAgentMode() ? '🤖 تم تفعيل وضع الوكيل' : '🤖 تم إيقاف وضع الوكيل');
}

function openResearchNow(){
  try{ openResearch(true); }catch(_){}
}
...
function resetChatModes(){
  try{ setDeepMode(false); }catch(_){}
  try{ setAgentMode(false); }catch(_){}
  try{ updateChatModeButtons(); }catch(_){}
  try{ updateChatModelBtn(); }catch(_){}
  try{ updateUsageBar(); }catch(_){}
  try{ renderChatModelMenu(); }catch(_){}
  ...
  function updateUsageBar({ lastIn=0, lastOut=0, lastCost=null } = {}){
    ...
    const modeTag = `${isDeepMode() ? '🧠 Deep' : ''}${(isDeepMode() && isAgentMode()) ? ' + ' : ''}${isAgentMode() ? '🤖 Agent' : ''}`.trim();
    const totals = readSessionUsage();
    const pricing = isOR ? getOpenRouterModelPricing(model) : null;

    const parts = [];
    parts.push(`النموذج: <b>${escapeHtml(model || '—')}</b>${modeTag ? ' • <b>'+escapeHtml(modeTag)+'</b>' : ''}`);
    if (lastIn || lastOut){
...
function getRecentModels(){
  ...
  const STATIC_REASONING_MODELS = ["deepseek/deepseek-r1", "deepseek/deepseek-r1-distill-llama-70b", "openai/o1", "openai/o1-mini", "openai/o3-mini", "qwen/qwq-32b", "google/gemini-2.5-pro", "anthropic/claude-3.7-sonnet"];
...
function showTab(name){
  ...
  try{ if (name === 'chat'){ applyModeUIRefresh(); } }catch(_){ }
}
...
// Modes
if (isDeepMode()){
  system += "\n\n[وضع التفكير العميق] التزم بالبنية التالية: (1) ملخص سريع (2) شرح مفصل (3) خطوات/أمثلة (4) نقاط تحقق/مخاطر (5) خلاصة.";
}
if (isAgentMode()){
    system += "\n\n[وضع الوكيل] ابدأ دائمًا بقسم: الخطة (3-6 خطوات مرقمة) ثم التنفيذ ثم النتيجة النهائية. إذا استخدمت الويب اذكر المصادر بروابط.";
}
...
async function runAgentMode(userGoal, statusBoxId, onDelta=null){
  const settings = getChatSettingsOrThrow();
  const baseUrl = settings.baseUrl || 'https://api.openai.com/v1';
  const isOR = isOpenRouterUrl(baseUrl);
  const signal = currentChatAbort?.signal || null;
  const fence = "```";
  ...
  // 1) Planner (JSON)
  setStatus(statusBoxId, '🤖 الوكيل: تخطيط...', true);
  const plannerSys =
    "أنت وكيل. أعد خطة قصيرة ثم اقترح حتى 3 استعلامات بحث إذا لزم.\n" +
    "أعد JSON فقط:\n" +
    '{"plan":["..."],"search":["..."],"deliverable":"(اختياري: اسم ملف)","notes":"..."}' + "\n" +
    "بدون أي نص خارج JSON.";

  let planObj = { plan: ['تحليل الهدف','بحث (إن لزم)','تنفيذ','نتيجة نهائية'], search: [], deliverable:'', notes:'' };
  try{
    const raw = await callChatCompletions({
      apiKey: settings.apiKey,
      baseUrl,
      model: settings.model,
      messages: [{ role:'system', content: plannerSys }, { role:'user', content: goal }],
      max_tokens: 900,
      temperature: 0.15,
      signal
    });
    const parsed = JSON.parse(String(raw||'').trim());
    if (parsed && typeof parsed === 'object') planObj = Object.assign(planObj, parsed);
  }catch(_){}

  // 2) Web search (optional) — OpenRouter :online or Gemini web search
  const allowGeminiWeb = (settings.chatProvider === 'gemini') && !!settings.geminiWebSearch;
  const allowOrWeb = isOR && !!settings.openrouterWebSearch;

  const results = [];
  const queries = (Array.isArray(planObj.search) ? planObj.search : []).map(x=>String(x||'').trim()).filter(Boolean).slice(0,3);

  if (queries.length && (allowGeminiWeb || allowOrWeb)){
    for (const q of queries){
      if (signal?.aborted) break;
      try{
        setStatus(statusBoxId, '🔎 بحث: ' + q, true);
        ...
        if (allowGeminiWeb){
          ...
          results.push({ query:q, text: out });
          continue;
        }

        let modelOnline = String(settings.model || '');
        if (!modelOnline.includes(':online')) modelOnline += ':online';

        const msg = [
          { role:'system', content: 'ابحث في الويب وأعد أهم 8-12 نقطة فقط، ثم اكتب قسم "المصادر:" بروابط مباشرة (URLs). لا تكتب نصًا طويلًا.' },
          { role:'user', content: q }
        ];
        const out = await callChatCompletions({ apiKey: settings.apiKey, baseUrl, model: modelOnline, messages: msg, max_tokens: 1600, temperature: 0.2, signal });
        results.push({ query:q, text: out });
      }catch(e){
        results.push({ query:q, text: 'فشل البحث: ' + (e?.message||e) });
      }
    }
  }

  // 3) Final synthesis (with attachments if any)
  setStatus(statusBoxId, '🧠 الوكيل: تنفيذ وكتابة النتيجة...', true);

  const planText = Array.isArray(planObj.plan) ? planObj.plan.map((x,i)=>`${i+1}) ${x}`).join('\n') : '1) تحليل\n2) تنفيذ';
  const researchText = results.length ? results.map(r => `### ${r.query}\n${String(r.text||'')}`).join('\n\n') : '';

  // Attachments: extracted text + images (vision)
  const parts = [];
  ...
  const deliver = String(planObj.deliverable || '').trim();
  const fileHint = deliver ? (`\n\nإن كان مناسبًا أنشئ ملفًا للتنزيل باسم "${deliver}" باستخدام:\n` + fence + `file name="${deliver}" mime="text/markdown"\n...content...\n` + fence) : '';

  let sys = 'أنت وكيل مساعد. اتبع القالب الإلزامي:\nالخطة:\n(خطة مرقمة قصيرة)\nالتنفيذ:\n(شرح منظم)\nالنتيجة النهائية:\n(مختصر واضح)\n';
  if (isDeepMode()){
    sys += '\n[تفكير عميق] زد التفصيل وأضف أمثلة وخطوات ومخاطر ونقاط تحقق.';
  }
  if (results.length){
    sys += '\nإن استخدمت معلومات من البحث، أضف قسم "المصادر:" بروابط.';
  }
  sys += fileHint;

  const userText =
    `المطلوب:\n${goal}\n\nالخطة المقترحة:\n${planText}\n\nنتائج البحث:\n${researchText || '(لا يوجد)'}\n\nملاحظات:\n${String(planObj.notes||'')}`;

  const messages = await buildChatMessagesForAPI(goal);
  // Replace the first system message with sys + existing system content (merge)
  ...
  // Final user message with parts (text + attachments)
  const userParts = parts.concat([{ type:'text', text: userText }]);
  ...
  ans = await callChatCompletions({
    ...
    model: settings.model,
    messages,
    max_tokens: Math.max(1600, settings.chatMaxOutTokens || 2000),
    temperature: 0.25,
    signal
  });

  // optional typewriter streaming
  const final = String(ans||'').trim();
  if (onDelta){
    let built='';
    const step = 36;
    for (let i=0;i<final.length;i+=step){
      if (signal?.aborted) break;
      built += final.slice(i, i+step);
      onDelta(final.slice(i, i+step), built);
      await sleep(12);
    }
  }
...
async function sendChat(userQuestion, statusBoxId){
    ...
    if (isAgentMode()){
      ...
      const out = await runAgentMode(userQuestion, statusBoxId, null);
      return out;
    }
    try{ await ensureSmartMemoryForProject(userQuestion); }catch(_){ }
...
async function sendChatStream(userText, statusBoxId, onDelta){
  ...
  if (isAgentMode()){
    try{ if (currentChatAbort) currentChatAbort.abort(); }catch(_){ }
    currentChatAbort = new AbortController();
    const final = await runAgentMode(userText, statusBoxId, onDelta);
    return final;
  }
...
function updateChatModelBtn(){
  ...
  const model = ($('model')?.value || '').trim();
  const icons = [];
    const onlineIcon = (model && String(model).includes(':online')) ? '🔎' : '';
    if (isDeepMode && isDeepMode()) icons.push('🧠');
    if (isAgentMode && isAgentMode()) icons.push('🤖');
    if (onlineIcon) icons.push(onlineIcon);
    const modeIcons = icons.length ? ('  ' + icons.join('')) : '';
    el.textContent = (isOR ? 'OR: ' : 'AI: ') + (model || '—') + modeIcons;
    try{ $('chatModelBtn') && ($('chatModelBtn').title = (model || '—') + (modeIcons?(' ' + icons.join(' ')):'') ); }catch(_){ }
}
...
function renderChatModelMenu(){
  ...
  // Base model toggle for :online
  if (isOR && current && String(current).includes(':online')){
    push(String(current).replace(':online',''), 'Base');
  }

  // Deep Search (OpenRouter :online variant)
  if (isOR && current && !String(current).includes(':online')){
    push(String(current) + ':online', 'Deep Search');
  }

  // Reasoning models (static shortlist)
  try{ (STATIC_REASONING_MODELS||[]).forEach(id => push(id, 'Reasoning')); }catch(_){ }
...
async function runCanvasAi(){
  ...
  const map={
    rewrite:'أعد صياغة النص ليصبح احترافيًا ومنظمًا.',
      build_app_html:'أنشئ تطبيق ويب HTML كامل في ملف واحد (يشمل CSS و JS داخل نفس الملف) وفق المطلوب. أعد الناتج كـ HTML فقط دون شرح.',
...
function refreshCanvasPreview(){
  const wrap = $('canvasPreviewWrap');
  const frame = $('canvasPreviewFrame');
  ...
  const htmlRaw = ($('canvasEditor')?.value || '');
  let html = htmlRaw;

  // If not full HTML, wrap inside basic template
  if (!isProbablyHtml(htmlRaw)){
    html = `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>Preview</title></head><body><pre style="white-space:pre-wrap;font-family:system-ui">${escapeHtml(htmlRaw)}</pre></body></html>`;
  }
...
// Final mode buttons (click + touchstart) - stable on Android
const bindBtn = (id, fn) => {
  const el = $(id);
  if (!el) return;
...
bindBtn('chatDeepBtn', toggleDeepModeNow);
bindBtn('chatAgentBtn', toggleAgentModeNow);
bindBtn('chatResearchBtn', openResearchNow);
bindBtn('chatModesOffBtn', disableAllModes);
try{ applyModeUIRefresh(); }catch(_){}


// Canvas preview
$('canvasPreviewToggle')?.addEventListener('change', refreshCanvasPreview);
$('canvasPreviewRefreshBtn')?.addEventListener('click', refreshCanvasPreview);


// Mode buttons
$('chatDeepBtn')?.addEventListener('click', ()=>{ setDeepMode(!isDeepMode()); updateChatModeButtons();
    $('chatModesResetBtn')?.addEventListener('click', (e)=>{ e.preventDefault(); resetChatModes(); });
 });
