...
function renderChatModelMenu(){
  ...
  const fav = (typeof getFavSet === 'function') ? Array.from(getFavSet()) : [];
  const recent = (typeof getRecentModels === 'function') ? getRecentModels() : [];
  ...
  filtered.slice(0, 140).forEach(item => {
    ...
    const favSet = (typeof getFavSet === 'function') ? getFavSet() : new Set();
    ...
    star.onclick = (e) => {
      e.stopPropagation();
      try{ toggleFavorite(item.id); }catch(_){}
      renderChatModelMenu();
    };
    ...
    pick.onclick = () => {
      ...
      if (mi){
        ...
        try{ addRecentModel(item.id); }catch(_){}
        updateChatModelBtn();
        closeChatModelMenu();
      }
    };
