/* Book Summarizer Pro v9 — OCR -> AI Fix -> Export + Knowledge Base (Embeddings RAG) */
(() => {
  const APP_VER = 9;

  const DB_NAME = 'book_summarizer_pro';
  const DB_VER  = 2; // v9 adds kb_docs + kb_chunks stores
  const STORE   = 'records';
  ...
  function loadSettings(){
    ...
    if ($('embedModel')) $('embedModel').value = saved.embedModel;
    if ($('language')) $('language').value = saved.language;
    if ($('ocrLang')) $('ocrLang').value = saved.ocrLang;
    if ($('aiFixMode')) $('aiFixMode').value = saved.aiFixMode;
    if ($('chatUseDoc')) $('chatUseDoc').value = saved.chatUseDoc;
    if ($('chatUseKB')) $('chatUseKB').value = saved.chatUseKB;
  ...
  function getSettingsOrThrow(){
    ...
    return { apiKey, baseUrl, model, embedModel, language };
  }
  ...
  async function sendChat(userQuestion, statusBoxId){
    ...
    const messages = await buildChatMessagesForAPI(userQuestion);
  ...
  function persistDraft(){
    ...
    localStorage.setItem('draft_v9', JSON.stringify(payload));
  }

  function restoreDraft(){
    try{
      const raw = localStorage.getItem('draft_v9');
      if (!raw) return;
  ...
  function persistChatDraft(){
    ...
    localStorage.setItem('chat_draft_v9', JSON.stringify(payload));
  }

  function restoreChatDraft(){
    try{
      const raw = localStorage.getItem('chat_draft_v9');
      if (!raw) return;
      const d = JSON.parse(raw);
...
<script>setTimeout(()=>window.print(), 450);</script>
  ...
  async function exportDocx(filename, title, text){
    ...
    const result = await fn(html, null, {
      title,
      creator: 'Book Summarizer Pro',
      direction: dir,
      lang,
      font: 'Arial'
    }, null);
  ...
  async function refreshKBUI(){
    const docs = await kbListDocs();
    const stats = $('kbStats');
    if (stats) stats.textContent = `قاعدة المعرفة: ${docs.length} ملف`;
  ...
  // -------------------- File parsing for KB --------------------
  async function parseFileToText(file, statusBoxId){
  ...
  function bindEvents(){
    ...
    $('kbManageBtn')?.addEventListener('click', async () => {
      await refreshKBUI();
      openKBDrawer(true);
    });
    ...
    $('kbFiles')?.addEventListener('change', async () => {
      ...
      try{
        ...
        for (let i=0;i<files.length;i++){
          ...
          await kbAddDocument({ name: f.name, type: (f.type || f.name.split('.').pop() || 'file'), text: t });
        }
    ...
    $('kbReindexBtn')?.addEventListener('click', async () => {
      try{
        await kbIndexAll('kbStatus');
      } catch(e){
        alert(e.message || String(e));
        setStatus('kbStatus', 'فشل الفهرسة.', true);
      }
    });
