...
async function fetchOpenRouterModels(){
  const res = await fetch('https://openrouter.ai/api/v1/models', { method:'GET', headers:{ 'Accept':'application/json' } });
  if (!res.ok){
  ...
  return arr.map(m => {
    ...
    return { id, name, context_length: ctx, pricing, pricing_prompt: pPrompt, pricing_completion: pComp, provider, modality, vision, tools, raw: m };
  }).filter(x => x.id);
}
  }
  const data = await res.json();
...
$('baseUrl') && ($('baseUrl').value = saved.baseUrl);
  ...
  function showTab(name){
    ...
    ['text','summary','chat','downloads','history'].forEach(n => {
      const el = $(`page-${n}`);
...
$('chatModelChip')?.addEventListener('click', openModelBrowser);
...
try{ updateChatModelChip(); }catch(_){}
$('model')?.addEventListener('change', () => { try{ updateChatModelChip(); }catch(_){ } });
$('baseUrl')?.addEventListener('change', () => { try{ updateChatModelChip(); }catch(_){ } });

// OpenRouter model list (auto-complete)
setTimeout(()=>ensureOpenRouterModelsLoaded({force:false}), 120);

$('orModelsRefreshBtn')?.addEventListener('click', async () => {
  await ensureOpenRouterModelsLoaded({force:true});
});
  ...
  if (st === 'true' && window.matchMedia('(min-width: 1100px)').matches === false){
    ...
    $('chatRegenBtn')?.addEventListener('click', async () => {
      try{
        ...
        const ans = await sendChat(lastUser.content, 'statusBox3');
...
if (settings.chatStreaming){
  ans = await sendChatStream(q, 'statusBox3', onDelta);
} else {
  ans = await sendChat(q, 'statusBox3');
}
...
async function renderModelBrowser(){
  ...
  try{
    ...
    if (status){ status.textContent = `الموديلات: ${items.length} • النتائج: ${filtered.length}`; }

    const curModel = ($('model')?.value || '').trim();

    filtered.slice(0,220).forEach(m => {
      const row=document.createElement('div'); row.className='mbRow';
      const left=document.createElement('div'); left.className='mbLeft';
      left.innerHTML = `
        <div class="mbName">${escapeHtml(m.id)}</div>
        <div class="mbMeta">
          ${m.provider ? `مزود: <b>${escapeHtml(m.provider)}</b> • ` : ''}
          سياق: <b>${escapeHtml(m.context_length ?? '—')}</b> •
          Prompt: <b>${escapeHtml(formatPrice(m.pricing_prompt))}</b> •
          Completion: <b>${escapeHtml(formatPrice(m.pricing_completion))}</b>
        </div>
      `;
      const tags=document.createElement('div'); tags.className='mbTags';
      if (m.vision) tags.innerHTML += `<span class="mbTag">Vision</span>`;
      if (m.tools) tags.innerHTML += `<span class="mbTag">Tools</span>`;
      if (m.modality) tags.innerHTML += `<span class="mbTag">${escapeHtml(String(m.modality))}</span>`;
      left.appendChild(tags);

      const actions=document.createElement('div'); actions.className='mbActions';
      const btn=document.createElement('button'); btn.type='button'; btn.className='btn primary sm';
      btn.textContent = (curModel===m.id)?'مُختار':'اختيار';
      btn.disabled = (curModel===m.id);
      btn.onclick = () => {
      ...
      const a=document.createElement('a');
      a.href=`https://openrouter.ai/models/${encodeURIComponent(m.id)}`;
      a.target='_blank'; a.rel='noopener';
      a.className='btn ghost sm'; a.textContent='عرض';
      actions.appendChild(btn); actions.appendChild(a);
    ...
    if (filtered.length>220){
      ...
      more.textContent='تم عرض أول 220 نتيجة فقط. استخدم البحث/الفلاتر لتقليل النتائج.';
...
function updateChatModelChip(){
  const el=$('chatModelText'); if (!el) return;
  const baseUrl=($('baseUrl')?.value || '').trim();
  const model=($('model')?.value || '').trim();
