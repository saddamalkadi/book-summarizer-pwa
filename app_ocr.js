  ...
  function bindEvents(){
    ...
    $('exportTemplateDocxBtn')?.addEventListener('click', async () => {
      try{
        ...
        if (!isPdf) return alert('التصدير (قالب مطابق) متاح لملفات PDF فقط حالياً.');
        // Use the latest corrected text (after AI Fix) if available, to attach it داخل الوورد كنسخة نصية قابلة للنسخ
        const corrected = ($('textInput')?.value || '').trim();
        await exportPdfTemplateDocxFromFile(src, corrected, 'statusBox');
        await saveRecord('export', 'تصدير Word (قالب من PDF)', '', { format:'docx', kind:'pdf_template', filename: src.name, ts: nowTs() });
        await renderHistory();
      } catch(e){ alert(e.message || String(e)); }
    });
