  ...
  try{
    if (!force && isFresh){
      populateOpenRouterDatalist(cache.items);
        try{ updateChatModelDropdown(); }catch(_){ }
...
$('chatModelBrowseBtn')?.addEventListener('click', openModelBrowser);
...
$('chatModelSelect')?.addEventListener('change', (e) => {
  ...
  if (!v) return;
  if (v === '__browse__'){ openModelBrowser(); try{ updateChatModelDropdown(); }catch(_){ } return; }
  if (v.startsWith('__sep')){ try{ updateChatModelDropdown(); }catch(_){ } return; }
  const mi = $('model');
  if (mi){
    mi.value = v;
    saveSettings();
...
function updateChatModelDropdown(){
  ...
  if (!isOR){
    sel.appendChild(makeOpt(current || '', current ? `AI: ${current}` : 'AI: —'));
    sel.value = current || '';
    return;
  }

  sel.appendChild(makeOpt(current || '', `OR: ${current || '—'}`));
  ...
  const cache = readOpenRouterModelsCache();
  const all = Array.isArray(cache?.items) ? cache.items : [];
  if (all.length){
