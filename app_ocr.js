/* Book Summarizer Pro v10 — Strategic multi-format ingestion + OCR -> AI Fix -> Export + Knowledge Base (Embeddings RAG)
...
(() => {
  ...
  function persistDraft(){
    ...
    localStorage.setItem('draft_v10', JSON.stringify(payload));
  }

  function restoreDraft(){
    try{
      const raw = localStorage.getItem('draft_v10');
      if (!raw) return;
  ...
  function persistChatDraft(){
    ...
    localStorage.setItem('chat_draft_v10', JSON.stringify(payload));
  }

  function restoreChatDraft(){
    try{
      const raw = localStorage.getItem('chat_draft_v10');
      if (!raw) return;
      const d = JSON.parse(raw);
