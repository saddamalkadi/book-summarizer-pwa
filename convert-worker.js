...
async function handleOcr(request, env) {
  ...
  const upstreamResp = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    headers: {
      ...
      "HTTP-Referer": String(env.OPENROUTER_REFERER || "https://app.saddamalkadi.com/"),
...
function withCors(response, env) {
  ...
  headers.set("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Client-Token, Cache-Control, Pragma, Accept, X-Requested-With");
